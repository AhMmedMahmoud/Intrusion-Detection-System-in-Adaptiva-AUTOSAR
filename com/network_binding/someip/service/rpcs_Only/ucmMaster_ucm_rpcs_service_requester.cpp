#include "ucmMaster_ucm_rpcs_service_requester.h"
    ...
    namespace com
    {
        namespace someip
        {
            namespace sd
            {
                ...
                const std::string UCMMaster_UCM_RPCSServiceRequester::cAnyIpAddress("0.0.0.0");
                ...
                */ 
                ...
                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::Activate(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cAcvtivateMethodID, mCounter, payload, data);
                }  

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::Finish(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cFinishMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetId(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetIdMethodID, mCounter, payload, data);
                }   

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetSwClusterChangeInfo(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetSwClusterChangeInfoMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetSwPackages(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetSwPackagesMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetSwClusterInfo(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetSwClusterInfoMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::RevertProcessedSwPackages(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cRevertProcessedSwPackagesMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::Rollback(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                ...
                */
                ...
                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::cancel(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cCancelMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::DeleteTransfer(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cDeleteTransferMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetHistory(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetHistoryMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetSwClusterManifestInfo(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetSwClusterManifestInfoMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::GetSwProcessProgress(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cGetSwProcessProgressMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::ProcessSwPackage(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cProcessSwPackageMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::TransferData(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cTransferDataMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::TransferExit(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                    return rpcClient->RequestWithoutHandler(mServiceId, cTransferExitMethodID, mCounter, payload, data);
                } 

                std::future<bool> UCMMaster_UCM_RPCSServiceRequester::TransferStart(const std::vector<uint8_t> &payload,
                                   std::vector<uint8_t> &data)
                {
                ...
                */
                ...
                bool UCMMaster_UCM_RPCSServiceRequester::init(uint16_t &_port)
                {
                    std::string ip;
                    uint16_t port;
                ...
                UCMMaster_UCM_RPCSServiceRequester::UCMMaster_UCM_RPCSServiceRequester(
                    uint16_t serviceId,
                    uint16_t instanceId,
                                                ...
                                                mUdpSocket(cAnyIpAddress, port, nicIpAddress, multicastGroup)
                {
                    ...
                    auto _receiver{std::bind(&UCMMaster_UCM_RPCSServiceRequester::onReceive, this)};
                    ...
                    auto _sender{std::bind(&UCMMaster_UCM_RPCSServiceRequester::onSend, this)};
                ...
                void UCMMaster_UCM_RPCSServiceRequester::InvokeOfferingHandler(sd::SomeIpSdMessage &&message)
                {
                ...
                bool UCMMaster_UCM_RPCSServiceRequester::findService()
                {
                ...
                bool UCMMaster_UCM_RPCSServiceRequester::tryExtractOfferedEndpoint(
                ...
                bool UCMMaster_UCM_RPCSServiceRequester::TryGetTransportInfo(int duration, std::string &ipAddress,uint16_t &port)
                {
                    bool _result;
                ...
                bool UCMMaster_UCM_RPCSServiceRequester::TryGetTransportInfo(std::string &ipAddress,uint16_t &port)
                {
                    bool _result;
                ...
                bool UCMMaster_UCM_RPCSServiceRequester::findService(uint16_t &_port)
                {
                    // create SOMEIP/SD message
                    SomeIpSdMessage mFindServieMessage;
                ...
                /**************************** poller functions  **********************************/
                void UCMMaster_UCM_RPCSServiceRequester::onReceive()
                {
                ...
                void UCMMaster_UCM_RPCSServiceRequester::onSend()
                {
                ...
                void UCMMaster_UCM_RPCSServiceRequester::Send(const SomeIpSdMessage &message)
                {
                ...
                AsyncBsdSocketLib::Poller* UCMMaster_UCM_RPCSServiceRequester::getPoller()
                {
                    return mPoller;
                }
