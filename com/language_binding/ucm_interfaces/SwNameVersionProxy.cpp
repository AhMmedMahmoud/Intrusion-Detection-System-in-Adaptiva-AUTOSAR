...
namespace ara 
{
    namespace com
    {
        ...
        {
                /******************** constructors ************************/ 
                requestedPackageProxy::HandleType::HandleType(InstanceIdentifier id,int16_t p): identifier{id},
                                                                                            port_no{p}
                {}

                /************ setters and getters *************************/
                void requestedPackageProxy::HandleType::setId(InstanceIdentifier id)
                {   identifier=id;  }

                void requestedPackageProxy::HandleType::setPort(int16_t p)
                {   port_no=p;  }

                int16_t requestedPackageProxy::HandleType::getPort()const
                {   return port_no; }

                const InstanceIdentifier& requestedPackageProxy::HandleType::GetInstanceId()const
                {   return identifier;  }

                void requestedPackageProxy::HandleType::setRequester(FieldServiceRequester *r)
                {   requester = r;  }

                FieldServiceRequester* requestedPackageProxy::HandleType::getRequester()
                { return requester; }  

                /*********************** static functions **********************/
                ServiceHandleContainer<requestedPackageProxy::HandleType> requestedPackageProxy::findSerivce(InstanceIdentifier id)
                {
                ...
                /************************* constructor **************************/
                requestedPackageProxy::requestedPackageProxy (HandleType &handle)
                {
                    requester = handle.getRequester();
                }

                /************************ fundemental funtions *******************/
                void requestedPackageProxy::subscribe(size_t maxSampleCount)
                {
                    requester->fieldClient->Subscribe(maxSampleCount);
                }

                helper::SubscriptionState requestedPackageProxy::GetSubscriptionState() const
                {
                    return requester->fieldClient->GetSubscriptionState();
                }

                std::future<bool> requestedPackageProxy::set(std::vector<uint8_t> &data)
                {
                    return requester->fieldClient->set(data);
                }

                std::future<bool> requestedPackageProxy::get(std::vector<uint8_t> &data)
                {
                    return requester->fieldClient->get(data);
                }

                void requestedPackageProxy::printSubscriptionState()
                {
                    requester->fieldClient->printCurrentState();
                }

                void requestedPackageProxy::SetReceiveHandler(Handler h)
                {
                    requester->fieldClient->SetReceiveHandler(h);
                }

                void requestedPackageProxy::UnsetReceiveHandler()
                {
                    requester->fieldClient->UnsetReceiveHandler();
                }

                /********************* deconstructor *******************/           
                requestedPackageProxy::~requestedPackageProxy()
                {
                    delete requester;
                }
        }
