...
namespace ara 
{
    namespace com
    {
        ...
        {    
                /******************** constructors ************************/ 
                currentStatusProxy::HandleType::HandleType(InstanceIdentifier id,int16_t p): identifier{id},
                                                                                            port_no{p}
                {}
                           
                /************ setters and getters *************************/
                void currentStatusProxy::HandleType::setId(InstanceIdentifier id)
                {   identifier=id;  }

                void currentStatusProxy::HandleType::setPort(int16_t p)
                {   port_no=p;  }

                int16_t currentStatusProxy::HandleType::getPort()const
                {   return port_no; }

                const InstanceIdentifier& currentStatusProxy::HandleType::GetInstanceId()const
                {   return identifier;  }

                void currentStatusProxy::HandleType::setRequester(FieldServiceRequester *r)
                {   requester = r;  }

                FieldServiceRequester* currentStatusProxy::HandleType::getRequester()
                { return requester; }  

                /*********************** static functions **********************/
                ServiceHandleContainer<currentStatusProxy::HandleType> currentStatusProxy::findSerivce(InstanceIdentifier id)
                {
                ...
                /************************* constructor **************************/
                currentStatusProxy::currentStatusProxy (HandleType &handle)
                {
                    requester = handle.getRequester();
                }

                /************************ fundemental funtions *******************/
                void currentStatusProxy::subscribe(size_t maxSampleCount)
                {
                    requester->fieldClient->Subscribe(maxSampleCount);
                }

                helper::SubscriptionState currentStatusProxy::GetSubscriptionState() const
                {
                    return requester->fieldClient->GetSubscriptionState();
                }

                std::future<bool> currentStatusProxy::set(std::vector<uint8_t> &data)
                {
                    return requester->fieldClient->set(data);
                }

                std::future<bool> currentStatusProxy::get(std::vector<uint8_t> &data)
                {
                    return requester->fieldClient->get(data);
                }

                void currentStatusProxy::printSubscriptionState()
                {
                    requester->fieldClient->printCurrentState();
                }

                void currentStatusProxy::SetReceiveHandler(Handler h)
                {
                    requester->fieldClient->SetReceiveHandler(h);
                }

                void currentStatusProxy::UnsetReceiveHandler()
                {
                    requester->fieldClient->UnsetReceiveHandler();
                }

                /********************* deconstructor *******************/            
                currentStatusProxy::~currentStatusProxy()
                {
                    delete requester;
                }
        }
