...
namespace ara 
{
    namespace com
    {
        ...
        {    
            /******************** constructors ************************/ 
            DENM_VDP_Proxy::HandleType::HandleType(InstanceIdentifier id,int16_t p): identifier{id},
                                                                                        port_no{p}
            {}

            /************ setters and getters *************************/
            void DENM_VDP_Proxy::HandleType::setId(InstanceIdentifier id)
            {   identifier=id;  }

            void DENM_VDP_Proxy::HandleType::setPort(int16_t p)
            {   port_no=p;  }

            int16_t DENM_VDP_Proxy::HandleType::getPort()const
            {   return port_no; }

            const InstanceIdentifier& DENM_VDP_Proxy::HandleType::GetInstanceId()const
            {   return identifier;  }

            void DENM_VDP_Proxy::HandleType::setRequester(EventServiceRequester *r)
            {   requester = r;  }

            EventServiceRequester* DENM_VDP_Proxy::HandleType::getRequester()
            { return requester; }  

            /*********************** static functions **********************/
            ServiceHandleContainer<DENM_VDP_Proxy::HandleType> DENM_VDP_Proxy::findSerivce(InstanceIdentifier id)
            {
            ...
            /************************* constructor **************************/
            DENM_VDP_Proxy::DENM_VDP_Proxy (HandleType &handle)
            {
                requester = handle.getRequester();
            }

            /************************ fundemental funtions *******************/
            void DENM_VDP_Proxy::subscribe(size_t maxSampleCount)
            {
                requester->eventClient->Subscribe(maxSampleCount);
            }

            helper::SubscriptionState DENM_VDP_Proxy::GetSubscriptionState() const
            {
